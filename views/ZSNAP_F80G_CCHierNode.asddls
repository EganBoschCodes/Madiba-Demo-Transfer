...
define view entity ZSNAP_F80G_CCHierNode
	as select from hrrp_node as main
	
	association [0..1] to ZSNAP_F80G_ControllingArea as _ControllingAreaText on $projection.ControllingArea = _ControllingAreaText.ControllingArea
	association [0..*] to ZSNAP_F80G_CCHierNodeT as _Text on $projection.CostCenterHierarchy = _Text.CostCenterHierarchy and $projection.HierarchyNode = _Text.HierarchyNode and $projection.ControllingArea = _Text.ControllingArea and $projection.CostCenter = ''
	association [0..*] to ZSNAP_F80G_CostCenter as _CostCenter on $projection.CostCenter = _CostCenter.CostCenter and $projection.ControllingArea = _CostCenter.ControllingArea
	association [1..1] to ZSNAP_F80G_CostCenterHier as _Hierarchy on $projection.CostCenterHierarchy = _Hierarchy.CostCenterHierarchy and $projection.ControllingArea = _Hierarchy.ControllingArea and $projection.ValidityEndDate = _Hierarchy.ValidityEndDate
