...
define view entity ZSNAP_F80G_GlobalCoHierNode
	as select from hrrp_node as main
	
	association [0..*] to ZSNAP_F80G_GlobalCoHierN_JG as _Text on $projection.GlobalCompanyHierarchy = _Text.GlobalCompanyHierarchy and $projection.HierarchyNode = _Text.HierarchyNode and $projection.Company = ''
	association [0..1] to ZSNAP_F80G_GlobalCompany as _Company on $projection.Company = _Company.Company
	association [1..1] to ZSNAP_F80G_GlobalCoHier as _Hierarchy on $projection.GlobalCompanyHierarchy = _Hierarchy.GlobalCompanyHierarchy and $projection.ValidityEndDate = _Hierarchy.ValidityEndDate
{
