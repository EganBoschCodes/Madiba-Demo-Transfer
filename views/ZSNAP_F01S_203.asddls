...
@EndUserText.label: 'SNAP F01: Cleared Documents'
	...
	as select from ZSNAP_F01S_050 (P_KeyDate: $parameters.P_KeyDate) as main
{
	...
	cast (dats_days_between (main.NetDueDate, main.ClearingDate) as abap.dec(10, 0)) as NetDueArrearsDaysInt,
	cast ($projection.NetDueArrearsDaysInt as abap.char(15)) as NetDueArrearsDays,
	case
		when $projection.DaysOverdueInt > 0 then 'X'
		else ''
	end as IsOverdue,
	cast (cast ($projection.DaysOverdueInt as abap.char(15)) as abap.numc(15)) as DaysOverdue,
	cast (case
		when main.ClearingDate > main.NetDueDate then dats_days_between (main.NetDueDate, main.ClearingDate)
		else 0
	end as abap.dec(10, 0)) as DaysOverdueInt,
	'C' as IsCleared,
