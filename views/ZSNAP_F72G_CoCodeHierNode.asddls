...
define view entity ZSNAP_F72G_CoCodeHierNode
	as select from hrrp_node as main
	
	inner join ZSNAP_F72G_CompanyCodeHier as HierarchyFilter on HierarchyFilter.CompanyCodeHierarchy = main.hryid and HierarchyFilter.ValidityEndDate = main.hryvalto
	
	association [0..*] to ZSNAP_F72G_CoCodeHierNodeT as _Text on $projection.CompanyCodeHierarchy = _Text.CompanyCodeHierarchy and $projection.HierarchyNode = _Text.HierarchyNode and $projection.CompanyCode = ''
	association [0..1] to ZSNAP_F72G_CompanyCode as _CompanyCode on $projection.CompanyCode = _CompanyCode.CompanyCode
	association [1..1] to ZSNAP_F72G_CompanyCodeHier as _Hierarchy on $projection.CompanyCodeHierarchy = _Hierarchy.CompanyCodeHierarchy and $projection.ValidityEndDate = _Hierarchy.ValidityEndDate
{
